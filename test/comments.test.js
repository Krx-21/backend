...
describe('Comment Routes', () => {
  ...
  beforeEach(async () => {
    await User.deleteOne({ email: 'cmt.provider@example.com' });
    const userRes = await request(app)
      .post('/api/v1/auth/register')
      .send({
        name: 'CMT - Provider User',
        telephoneNumber: '0696969696',
        email: 'cmt.provider@example.com',
        password: 'testpass',
        role: 'provider'
      });
    ...
    await User.deleteOne({ email: 'cmt.user@example.com' });
    const userRes2 = await request(app)
      .post('/api/v1/auth/register')
      .send({
        name: 'CMT - Regular User',
        telephoneNumber: '1111111111',
        email: 'cmt.user@example.com',
    ...
    createdIds.users.push(commentUserId);
		...
		await Car.deleteOne({ brand: 'Toyota', model: 'Vios' });
    const car = await Car.create({
      ...
      year: 2022,
      pricePerDay: 60,
      carDescription: 'Agile family car',
  ...
  describe('POST /api/v1/comments', () => {
    it('should create a comment by a user', async () => {
      ...
      expect(res.body.success).toBe(true);
		...
		it('should return 400 if the user has not booked the car', async () => {
		...
		it('should return 404 to add comment on non existing car', async () => {
			const nonExistentCarId = new mongoose.Types.ObjectId();
			const res = await request(app)
	...
	describe('GET /api/v1/comments/:carId', () => {
	...
	describe('PUT /api/v1/comments/:commentId', () => {
		...
		it('should return 404 if the user try to update comment that doesn\'t exist', async () => {
