...
describe('Booking Routes', () => {
  ...
  describe('POST /api/v1/cars/:carId/bookings with promotions', () => {
    ...
    // it('should not apply a promotion with mismatched provider', async () => {
    //   // Create a promotion for a different provider
    //   const otherProviderRes = await request(app)
    //     .post('/api/v1/auth/register')
    //     .send({
    //       name: 'Other Provider',
    ...
    //   const otherProviderToken = otherProviderRes.body.token;
    //   const otherProviderId = otherProviderRes.body.data.myRcpId;
    ...
    // it('should not apply an expired promotion', async () => {
    //   // Create an expired promotion
    //   const expiredPromoRes = await request(app)
    //     .post('/api/v1/promotions')
    //     .set('Authorization', `Bearer ${token}`)
    //     .send({
    //       title: 'Expired Promo',
    ...
    //   const expiredPromoId = expiredPromoRes.body.data._id;
    ...
    // it('should not apply a promotion if base price is below minimum purchase amount', async () => {
    //   const res = await request(app)
    //     .post(`/api/v1/cars/${carId}/bookings`)
    //     .set('Authorization', `Bearer ${regUserToken}`)
    //     .send({
    //       start_date: new Date(Date.now() + 86400000).toISOString(),
    //       end_date: new Date(Date.now() + 86400000 * 1).toISOString(), // 1 day booking
    //       promoId,
    //     });
  
    //   expect(res.statusCode).toBe(201);
    //   expect(res.body.data.totalprice).toBe(1200); // No discount applied
    // });

    // it('should not apply a promotion with no remaining uses', async () => {
    //   // Deplete the promotion's uses
    //   for (let i = 0; i < 5; i++) {
    //     await request(app)
    //       .post(`/api/v1/cars/${carId}/bookings`)
    //       .set('Authorization', `Bearer ${regUserToken}`)
    ...
    //   expect(res.statusCode).toBe(201);
    //   expect(res.body.data.totalprice).toBe(1200 * 1); // No discount applied
    // });
  });
});
